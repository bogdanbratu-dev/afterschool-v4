// Engine crawler per-domeniu, la nivel de cartier, pentru COLABORATORI (kind=institutie, sursa Google Maps).
// Regula de aur: se insereaza DOAR rezultate cu ADRESA STRADALA REALA. Fara adresa valida -> skip.
// Utilizare: node scrape-collaborators.js <domeniu> [nr_cartiere]
...
const LOG_PATH = path.join(__dirname, '..', 'data', `scrape-collab-${DOMAIN}.log`);
...
// Fiecare domeniu: template-uri de cautare (Maps) => kind institutie
const DOMAIN_CFG = {
  limbi_straine: ['scoala de limbi straine copii {c} Bucuresti','centru de limbi straine {c} Bucuresti'],
  robotica: ['club robotica copii {c} Bucuresti','cursuri programare copii {c} Bucuresti'],
  sah: ['club de sah copii {c} Bucuresti'],
  soroban: ['curs soroban {c} Bucuresti','aritmetica mentala copii {c} Bucuresti'],
  stiinte: ['atelier de stiinte copii {c} Bucuresti'],
  meditatii: ['centru de meditatii {c} Bucuresti','centru educational {c} Bucuresti'],
  muzica: ['scoala de muzica copii {c} Bucuresti'],
  arta: ['atelier de pictura copii {c} Bucuresti','scoala de arte copii {c} Bucuresti'],
  teatru: ['teatru pentru copii {c} Bucuresti','cursuri de actorie copii {c} Bucuresti'],
  dans: ['scoala de dans copii {c} Bucuresti'],
  sport_indoor: ['gimnastica copii {c} Bucuresti','arte martiale copii {c} Bucuresti'],
  yoga: ['yoga pentru copii {c} Bucuresti'],
  gatit: ['atelier culinar copii {c} Bucuresti'],
  logopedie: ['cabinet logopedie {c} Bucuresti'],
  psihologie: ['cabinet psihologie copii {c} Bucuresti'],
  terapie: ['kinetoterapie copii {c} Bucuresti','terapie copii {c} Bucuresti'],
  foto_video: ['studio foto {c} Bucuresti'],
  public_speaking: ['curs public speaking copii {c} Bucuresti'],
  lectura: ['club de lectura copii {c} Bucuresti'],
  educatie_financiara: ['educatie financiara copii {c} Bucuresti'],
  caligrafie: ['atelier caligrafie {c} Bucuresti'],
  dezvoltare_personala: ['dezvoltare personala copii {c} Bucuresti'],
};

if (!DOMAIN || !DOMAIN_CFG[DOMAIN]) { console.error('Domeniu invalid. Optiuni:', Object.keys(DOMAIN_CFG).join(', ')); process.exit(1); }
...
// VALIDARE ADRESA STRADALA REALA
...
function validAddress(a){
  ...
  if (STREET_START.test(a)) return true;
  // București proper only - reject Ilfov suburbs
  const ILFOV = /\b(ilfov|chiajna|bragadiru|ro[sş]u|m[ăa]gurele|jilava|glina|chitila|dobroie[sş]ti|balotesti|corbeanca|mogosoaia|1\s*decembrie)\b/i;
  if (ILFOV.test(a)) return false;
  const hasNr = /\d/.test(a);
  const hasLoc = /(sector\s*[1-6]|bucure[sșş]ti|voluntari|otopeni|pantelimon|pope[sş]ti)/i.test(a);
  return hasNr && hasLoc;
}
...
const insert = db.prepare(`
  INSERT INTO professionals (name, category, kind, address, sector, lat, lng, phone, email, website, maps_url, description, availability, online_available, home_service, is_premium, is_featured, contacts_hidden)
  VALUES (?, ?, 'institutie', ?, ?, ?, ?, ?, ?, ?, ?, ?, 'unknown', 0, 0, 0, 0, 0)
`);

let SKIPPED_NOADDR = 0;
...
async function scrapeQuery(page, query, seen){
  ...
  for(let i=0;i<cap;i++){
    try{
      ...
      // REGULA DE AUR: adresa stradala reala obligatorie
      if(!validAddress(address)){
        SKIPPED_NOADDR++;
        log(`  ⏭️  fara adresa valida: ${name} (addr="${address}")`);
        await page.goBack({waitUntil:'domcontentloaded',timeout:10000}).catch(()=>{}); await sleep(1400);
        continue;
      }
      // coords in Bucuresti+Ilfov
      if(coords && (coords.lat<44.30||coords.lat>44.60||coords.lng<25.90||coords.lng>26.30)){
        await page.goBack({waitUntil:'domcontentloaded',timeout:10000}).catch(()=>{}); await sleep(1400); continue;
      }
      const fc=coords||{lat:0,lng:0};
      const sector=extractSector(address);
      const mapsUrl = url.split('?')[0];
      insert.run(name.trim(), DOMAIN, address, sector, fc.lat, fc.lng, phone, null, website, mapsUrl, `${name.trim()} in Bucuresti${sector?', Sector '+sector:''}.`);
      seen.add(nn); added++;
...
async function main(){
  const templates = DOMAIN_CFG[DOMAIN];
  ...
  log(`📦 domeniu=${DOMAIN} | ${existing.length} existenti | ${cartiere.length} cartiere x ${templates.length} template-uri\n`);
  ...
  for(const cartier of cartiere){
    for(const t of templates){
      const q=t.replace('{c}', cartier);
      try{ total+=await scrapeQuery(page,q,seen); }catch(e){ log('❌ '+String(e.message).substring(0,80)); }
      await sleep(1500);
    }
  }
  await browser.close();
  const cnt=db.prepare('SELECT COUNT(*) as c FROM professionals WHERE category=?').get(DOMAIN);
  log(`\n✅ [${DOMAIN}] NOI: ${total} | sarite fara adresa: ${SKIPPED_NOADDR} | total in categorie: ${cnt.c}`);
}
