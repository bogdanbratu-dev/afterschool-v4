// Crawler CATERING la nivel de cartier (Google Maps). Regula de aur: adresa stradala reala obligatorie.
// Utilizare: node scrape-catering-cartier.js [nr_cartiere]
...
const LOG_PATH = path.join(__dirname, '..', 'data', 'scrape-catering-cartier.log');
...
const TEMPLATES = ['firma de catering {c} Bucuresti','catering mancare {c} Bucuresti'];
...
const insert = db.prepare(`
  INSERT INTO caterers (name, address, sector, lat, lng, coverage_area, phone, email, website, maps_url, description, availability, is_premium, is_featured, contacts_hidden)
  VALUES (?, ?, ?, ?, ?, ?, ?, NULL, ?, ?, ?, 'unknown', 0, 0, 0)
`);
let SKIP_NOADDR = 0;

async function scrapeQuery(page, query, cartier, seen){
  log(`\n🔍 "${query}"`);
  ...
  for(let i=0;i<cap;i++){
    try{
      ...
      insert.run(name.trim(), address, sector, fc.lat, fc.lng, cartier, phone, website, url.split('?')[0], `Firma de catering in Bucuresti${sector?', Sector '+sector:''}. Zona: ${cartier}.`);
      seen.add(nn); added++;
...
async function main(){
  ...
  log(`📦 ${existing.length} caterers existenti | ${cartiere.length} cartiere x ${TEMPLATES.length} template-uri\n`);
  ...
  let total=0;
  for(const cartier of cartiere){ for(const t of TEMPLATES){ try{ total+=await scrapeQuery(page,t.replace('{c}',cartier),cartier,seen); }catch(e){ log('❌ '+String(e.message).substring(0,80)); } await sleep(1400); } }
  await browser.close();
  const c=db.prepare('SELECT COUNT(*) c FROM caterers').get();
  log(`\n✅ NOI: ${total} | sarite fara adresa: ${SKIP_NOADDR} | total caterers: ${c.c}`);
}
