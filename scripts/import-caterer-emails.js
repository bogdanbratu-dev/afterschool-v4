/**
 * import-caterer-emails.js — RULAT PE VPS. Citeste data/caterer-emails.json (produs local de
 * scrape-caterer-emails.js prin Method 2) si scrie emailurile in tabela `caterers`.
 *
 ...
 *
 *   node scripts/import-caterer-emails.js --dry-run   # arata ce ar face
 *   node scripts/import-caterer-emails.js             # aplica (fa backup DB inainte!)
 */
...
const FILE = path.join(__dirname, '..', 'data', 'caterer-emails.json');
...
const PLACEHOLDER_DOMAINS = ['exemplu.ro', 'example.com', 'example.ro', 'example.org',
  'mysite.com', 'domain.com', 'domainname.com', 'email.com', 'yourdomain.com', 'test.ro', 'test.com',
  'siteulmeu.com'];
...
function main() {
  ...
  const getCur = db.prepare('SELECT id, name, email FROM caterers WHERE id = ?');
  const upd = db.prepare("UPDATE caterers SET email = ? WHERE id = ? AND (email IS NULL OR TRIM(email) = '')");
  ...
  const totalWithEmail = db.prepare("SELECT COUNT(*) n FROM caterers WHERE email IS NOT NULL AND TRIM(email) != ''").get().n;
  db.close();
  console.log(`\n[import] ${DRY ? 'ar actualiza' : 'actualizate'}=${updated} sarite(au deja email)=${skippedHasEmail} lipsa_in_db=${missing}`);
  console.log(`[import] caterari cu email acum: ${totalWithEmail}${DRY ? ' (dupa aplicare)' : ''}`);
}
