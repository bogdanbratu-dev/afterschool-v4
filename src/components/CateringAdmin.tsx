...
type FormState = Partial<Caterer>;

const EMPTY: FormState = {
  name: '', address: '', sector: 1, lat: 0, lng: 0, coverage_area: '',
...
export default function CateringAdmin() {
  ...
  const openEdit = async (c: Caterer) => {
    setEditing(c); setForm({ ...c });
    try {
      const mr = await fetch(`/api/admin/microsites?listing_type=caterer&listing_id=${c.id}`);
  ...
  const save = async (e: React.FormEvent) => {
    e.preventDefault();
    const url = editing ? `/api/admin/caterers/${editing.id}` : '/api/admin/caterers';
  ...
  const remove = async (id: number) => {
    if (!confirm('Stergi aceasta firma de catering?')) return;
    await fetch(`/api/admin/caterers/${id}`, { method: 'DELETE' });
    load();
  };

  const togglePremium = async (c: Caterer) => {
    if (!c.is_premium && c.sector != null) {
      const satRes = await fetch('/api/admin/saturation');
      const satData = await satRes.json();
      const row = (satData.rows || []).find((r: any) => r.table === 'caterers' && r.sector === c.sector);
      if (row && row.untilHalf <= 1 && row.total >= 4) {
        ...
        const msg = row.untilHalf <= 0
          ? `Atentie: Sector ${c.sector} este deja la ${row.premium} din ${row.total} firme catering premium (${pct}%). Zona e deja depasita.\n\nContinui totusi?`
          : `Atentie: aceasta vanzare va pune Sector ${c.sector} la ${newPremium} din ${row.total} firme catering premium (${pct}%), depasind pragul.\n\nContinui totusi?`;
        if (!window.confirm(msg)) return;
      }
    }
    await fetch(`/api/admin/caterers/${c.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...c, is_premium: c.is_premium ? 0 : 1 }) });
    load();
  }
  const toggleFeatured = async (c: Caterer) => {
    await fetch(`/api/admin/caterers/${c.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...c, is_featured: c.is_featured ? 0 : 1 }) });
    load();
  };
  ...
  const bulkSetContactsHidden = async (hidden: boolean) => {
    setBulkLoading(true);
    await fetch('/api/admin/caterers', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids: [...selectedIds], contacts_hidden: hidden }) });
  ...
  const quickCreateMicrosite = async (id: number) => {
    ...
    const res = await fetch('/api/admin/microsites/quick-create', {
      ...
      body: JSON.stringify({ listing_type: 'caterer', listing_id: id }),
    });
    ...
    <div>
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        <h2 className="text-xl font-bold text-[var(--color-text-main)]">
          🍽️ Catering <span className="text-sm font-normal text-[var(--color-text-light)]">({filtered.length}{search ? ` din ${caterers.length}` : ''})</span>
        </h2>
      ...
      <div className="bg-[var(--color-card)] rounded-xl border border-[var(--color-border)] overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[var(--color-border)]">
            {filtered.length === 0 ? (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-[var(--color-text-light)]">Nicio firma de catering. Apasa „+ Adauga".</td></tr>
            ) : filtered.map(c => (
              <tr key={c.id} className="text-[var(--color-text-main)] hover:bg-[var(--color-bg)]">
                ...
                <td className="px-4 py-3">
                  <a href={`/catering/${c.name.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}-${c.id}`} target="_blank" rel="noopener" className="font-medium hover:text-teal-600">{c.name}</a>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-10 px-4">
          ...
          <div className="relative bg-[var(--color-card)] rounded-xl shadow-xl w-full max-w-2xl max-h-[85vh] overflow-y-auto p-6">
            <h3 className="text-lg font-bold mb-4 text-[var(--color-text-main)]">{editing ? 'Editează firma de catering' : 'Adaugă firmă de catering'}</h3>
            <form onSubmit={save} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium mb-1">Zone deservite (ex: Sector 1, 2, 3, Pipera, Voluntari)</label>
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Pret minim (lei/portie)</label>
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Pret maxim (lei/portie)</label>
