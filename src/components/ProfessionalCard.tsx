...
import { PROFESSIONAL_CATEGORY_LABELS, type ProfessionalCategory, type CollaboratorKind } from '@/lib/professionals';
...
export default function ProfessionalCard({ data, businessMode }: ProfessionalCardProps) {
  const contactHidden = !!data.contacts_masked;
  const isSpotlight = !!data.is_spotlight;

  const trackUrl = (lt: string, dest: string) =>
    `/api/track?type=professional&id=${data.id}&name=${encodeURIComponent(data.name)}&lt=${lt}&url=${encodeURIComponent(dest)}`;
      ...
      <div className="p-3 sm:p-3.5">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="font-bold text-base sm:text-lg text-[var(--color-text-main)] leading-tight">
                <a href={`/colaboratori/${toSlug(data.name, data.id)}`} className="hover:text-teal-600 transition-colors">{data.name}</a>
              </h3>
              {data.is_featured === 1 && (
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          <span className="inline-flex items-center gap-1 bg-indigo-50 text-indigo-700 border border-indigo-200 px-3 py-1 rounded-full text-xs font-semibold">
            {PROFESSIONAL_CATEGORY_LABELS[data.category] || data.category}
          </span>
          {data.coverage_area && (
