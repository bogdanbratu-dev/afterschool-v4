...
import { TUTOR_SUBJECT_LABELS, TUTOR_KIND_LABELS, type TutorSubject } from '@/lib/tutors';
...
export default function TutorCard({ data, businessMode }: TutorCardProps) {
  const contactHidden = !!data.contacts_masked;
  const isSpotlight = !!data.is_spotlight;

  const trackUrl = (lt: string, dest: string) =>
    `/api/track?type=tutor&id=${data.id}&name=${encodeURIComponent(data.name)}&lt=${lt}&url=${encodeURIComponent(dest)}`;
      ...
      <div className="p-3 sm:p-3.5">
        <div className="flex items-start justify-between mb-2">
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="font-bold text-base sm:text-lg text-[var(--color-text-main)] leading-tight">
                <a href={`/meditatii/${toSlug(data.name, data.id)}`} className="hover:text-teal-600 transition-colors">{data.name}</a>
              </h3>
              {data.is_featured === 1 && (
          ...
          <div className="flex flex-col items-end gap-1 flex-shrink-0 ml-2">
            ...
            <span className="text-lg" title="Colaborator">👨‍🏫</span>
          </div>
        </div>

        {/* Categorie + badge-uri online / la domiciliu */}
        <div className="flex flex-wrap gap-2 mb-2">
          <span className="inline-flex items-center gap-1 bg-indigo-50 text-indigo-700 border border-indigo-200 px-3 py-1 rounded-full text-xs font-semibold">
            {TUTOR_SUBJECT_LABELS[data.subject] || data.subject}
          </span>
          {data.kind && (
            <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold border ${data.kind === 'independent' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>
              {data.kind === 'independent' ? '🚗 ' : '🏢 '}{TUTOR_KIND_LABELS[data.kind]}
            </span>
          )}
          {data.online_available === 1 && (
            <span className="inline-flex items-center gap-1 bg-sky-50 text-sky-700 border border-sky-200 px-3 py-1 rounded-full text-xs font-semibold">💻 Online</span>
          )}
          {data.home_service === 1 && (
            <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 border border-emerald-200 px-3 py-1 rounded-full text-xs font-semibold">🏠 La domiciliu</span>
          )}
          {data.coverage_area && (
        ...
        {data.price_min !== null && (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-2">
            <div className="bg-green-50 rounded-lg p-1.5 text-center">
              <div className="text-xs text-[var(--color-text-light)]">Pret/ora</div>
