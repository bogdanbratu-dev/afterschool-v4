...
type FormState = Partial<Professional>;

const EMPTY: FormState = {
  name: '', category: 'altele', kind: 'institutie', address: '', sector: 1, lat: 0, lng: 0, coverage_area: '',
  ...
  availability: 'unknown', online_available: 0, home_service: 0, is_premium: 0, premium_expires_at: null, is_featured: 0, contacts_hidden: 0,
};
...
export default function ProfessionalsAdmin() {
  ...
  const openEdit = async (c: Professional) => {
    setEditing(c); setForm({ ...c });
    try {
      const mr = await fetch(`/api/admin/microsites?listing_type=professional&listing_id=${c.id}`);
  ...
  const save = async (e: React.FormEvent) => {
    e.preventDefault();
    const url = editing ? `/api/admin/professionals/${editing.id}` : '/api/admin/professionals';
  ...
  const remove = async (id: number) => {
    if (!confirm('Stergi acest colaborator?')) return;
    await fetch(`/api/admin/professionals/${id}`, { method: 'DELETE' });
    load();
  };

  const togglePremium = async (c: Professional) => {
    if (!c.is_premium && c.sector != null) {
      const satRes = await fetch('/api/admin/saturation');
      const satData = await satRes.json();
      const row = (satData.rows || []).find((r: any) => r.table === 'professionals' && r.sector === c.sector);
      if (row && row.untilHalf <= 1 && row.total >= 4) {
        ...
        const msg = row.untilHalf <= 0
          ? `Atentie: Sector ${c.sector} este deja la ${row.premium} din ${row.total} colaboratori premium (${pct}%). Zona e deja depasita.\n\nContinui totusi?`
          : `Atentie: aceasta vanzare va pune Sector ${c.sector} la ${newPremium} din ${row.total} colaboratori premium (${pct}%), depasind pragul.\n\nContinui totusi?`;
        if (!window.confirm(msg)) return;
      }
    }
    await fetch(`/api/admin/professionals/${c.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...c, is_premium: c.is_premium ? 0 : 1 }) });
    load();
  }
  const toggleFeatured = async (c: Professional) => {
    await fetch(`/api/admin/professionals/${c.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...c, is_featured: c.is_featured ? 0 : 1 }) });
    load();
  };
  ...
  const bulkSetContactsHidden = async (hidden: boolean) => {
    setBulkLoading(true);
    await fetch('/api/admin/professionals', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids: [...selectedIds], contacts_hidden: hidden }) });
  ...
  const quickCreateMicrosite = async (id: number) => {
    ...
    const res = await fetch('/api/admin/microsites/quick-create', {
      ...
      body: JSON.stringify({ listing_type: 'professional', listing_id: id }),
    });
    ...
    <div>
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        <h2 className="text-xl font-bold text-[var(--color-text-main)]">
          👨‍🏫 Colaboratori <span className="text-sm font-normal text-[var(--color-text-light)]">({filtered.length}{search ? ` din ${professionals.length}` : ''})</span>
        </h2>
        ...
        <table className="w-full text-sm">
          <thead className="bg-[var(--color-bg)] text-[var(--color-text-light)]">
            <tr>
              ...
              <th className="px-4 py-3 text-left hidden md:table-cell">Zone deservite</th>
          ...
          <tbody className="divide-y divide-[var(--color-border)]">
            {filtered.length === 0 ? (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-[var(--color-text-light)]">Niciun colaborator. Apasa „+ Adauga".</td></tr>
            ) : filtered.map(c => (
              <tr key={c.id} className="text-[var(--color-text-main)] hover:bg-[var(--color-bg)]">
                ...
                <td className="px-4 py-3">
                  <a href={`/colaboratori/${c.name.toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')}-${c.id}`} target="_blank" rel="noopener" className="font-medium hover:text-teal-600">{c.name}</a>
                  <div className="text-xs text-[var(--color-text-light)]">{c.address}</div>
                </td>
                <td className="px-4 py-3 hidden md:table-cell text-xs text-[var(--color-text-light)]">{c.coverage_area || '—'}</td>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-10 px-4">
          ...
          <div className="relative bg-[var(--color-card)] rounded-xl shadow-xl w-full max-w-2xl max-h-[85vh] overflow-y-auto p-6">
            <h3 className="text-lg font-bold mb-4 text-[var(--color-text-main)]">{editing ? 'Editează colaborator' : 'Adaugă colaborator'}</h3>
            <form onSubmit={save} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <select required value={form.category || 'altele'} onChange={e => setForm({ ...form, category: e.target.value as ProfessionalCategory })} className={inputCls}>
                    {PROFESSIONAL_CATEGORY_ORDER.map(cat => <option key={cat} value={cat}>{PROFESSIONAL_CATEGORY_LABELS[cat]}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Tip colaborator *</label>
                  <select value={form.kind || 'independent'} onChange={e => setForm({ ...form, kind: e.target.value as 'independent' | 'institutie' })} className={inputCls}>
                    <option value="independent">Se deplaseaza (independent)</option>
                    <option value="institutie">La sediu (institutie)</option>
                  </select>
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium mb-1">Adresa (optional - lasa gol daca lucreaza online)</label>
                  <input value={form.address || ''} onChange={e => setForm({ ...form, address: e.target.value })} className={inputCls} />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium mb-1">Zona acoperita (ex: Sector 1, 2, 3, Pipera, online)</label>
                  <input value={form.coverage_area || ''} onChange={e => setForm({ ...form, coverage_area: e.target.value })} placeholder="Sector 1, 2, 3, Pipera" className={inputCls} />
                </div>
                <div>
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Pret minim (lei/sedinta)</label>
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Pret maxim (lei/sedinta)</label>
              ...
              {!!form.is_premium && (
                <div className="ml-6 mt-1 flex flex-wrap items-center gap-2">
                  ...
                  <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={!!form.online_available} onChange={e => setForm({ ...form, online_available: e.target.checked ? 1 : 0 })} /> 💻 Online</label>
                  <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={!!form.home_service} onChange={e => setForm({ ...form, home_service: e.target.checked ? 1 : 0 })} /> 🏠 La domiciliu</label>
              ...
              {editing && editMicrosite && (
                <div className="mt-2 border border-teal-200 rounded-xl p-4 bg-teal-50/40 space-y-3">
                  <p className="text-sm font-semibold text-teal-800">Outreach colaborator</p>
                  ...
                  {editMicrosite.outreach_enabled === 1 && <>
                    ...
                    <div>
                      ...
                      <input type="email" value={editMicrosite.outreach_from_email} onChange={e => setEditMicrosite(m => m ? { ...m, outreach_from_email: e.target.value } : m)}
                        placeholder="contact@colaborator.ro" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white" />
                    </div>
                  </>}
                </div>
              )}
