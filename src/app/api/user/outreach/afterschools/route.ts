...
export async function GET() {
  ...
  const afterschools = db.prepare(`
    SELECT a.id, a.name, a.sector, a.neighborhood, a.email, a.phone,
           oc.status as outreach_status, oc.email_sent_at
    FROM afterschools a
    LEFT JOIN outreach_contacts oc ON oc.listing_type = 'afterschool' AND oc.listing_id = a.id AND oc.partner_ms_id = ?
  ...
  const sectors = buildGroups(afterschools, (a) => String(a.sector || 0));
  const neighborhoods = buildGroups(afterschools, (a) => a.neighborhood || 'necunoscut');
  const ownLocation = getOwnLocation(db, ms);
  const batches = getSavedBatches(db, ms.id as number, 'afterschool', afterschools, (a) => String(a.sector || 0), (a) => a.neighborhood);
