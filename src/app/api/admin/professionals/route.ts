...
export async function POST(request: Request) {
  ...
  const info = db.prepare(`
    INSERT INTO professionals (name, category, kind, address, sector, lat, lng, coverage_area, phone, email,
      ...
      online_available, home_service, is_premium, is_featured, contacts_hidden)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    b.name, b.category || 'altele', b.kind || 'independent', b.address || null, b.sector || null, b.lat || 0, b.lng || 0,
    ...
    b.online_available ? 1 : 0, b.home_service ? 1 : 0,
...
export async function PATCH(request: Request) {
  ...
  const db = getDb();
  const stmt = db.prepare('UPDATE professionals SET contacts_hidden = ? WHERE id = ?');
