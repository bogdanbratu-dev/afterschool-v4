...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  db.prepare(`
    UPDATE caterers SET
      name = ?, address = ?, sector = ?, lat = ?, lng = ?, neighborhood = ?, coverage_area = ?,
      ...
      availability = ?, is_premium = ?, premium_expires_at = ?, is_featured = ?, contacts_hidden = ?, leads_enabled = ?
    WHERE id = ?
  `).run(
    b.name, b.address, sector, lat, lng, neighborhood, b.coverage_area || null,
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const db = getDb();
  db.prepare('DELETE FROM caterers WHERE id = ?').run(id);
  return NextResponse.json({ success: true });
}
