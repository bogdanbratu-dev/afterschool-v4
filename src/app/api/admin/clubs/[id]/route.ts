...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  db.prepare(`
    UPDATE clubs SET
      ...
      availability = ?, is_premium = ?, premium_expires_at = ?, contacts_hidden = ?, leads_enabled = ?,
      banner_url = ?,
  ...
  `).run(
    ...
    body.is_premium ?? 0, body.premium_expires_at ?? null, body.contacts_hidden ?? 0, body.leads_enabled ?? null,
