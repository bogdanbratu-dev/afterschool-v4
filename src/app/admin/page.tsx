        ...
        {activeTab === 'analytics' && (
          <div>
            ...
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold">Statistici vizitatori</h2>
              <div className="flex gap-2">
                {[7, 30, 90].map(d => (
                  <button key={d} onClick={() => { setAnalyticsDays(d); loadAnalytics(d); }}
                    className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${analyticsDays === d ? 'bg-[var(--color-primary)] text-white' : 'bg-gray-100 text-[var(--color-text-light)] hover:bg-gray-200'}`}>
                    {d} zile
                  </button>
                ))}
              </div>
            ...
            {analyticsLoading ? (
              ...
              <p className="text-center text-[var(--color-text-light)] py-16">Nu există date încă.</p>
            ) : (
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl border border-[var(--color-border)] p-5">
                  ...
                  {(() => {
                    const max = Math.max(...analyticsData.visitsByDay.map(d => d.count), 1);
                    return (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-white rounded-xl border border-[var(--color-border)] p-5">
                    ...
                    {(() => {
                      ...
                      return Object.entries(analyticsData.pageBreakdown).map(([page, count]) => (
                        <div key={page} className="mb-3">
                          ...
                          <div className="w-full bg-gray-100 rounded-full h-2">
                            <div className="bg-[var(--color-primary)] h-2 rounded-full" style={{ width: `${count / total * 100}%` }} />
                          </div>
                        </div>
                      ));
                    })()}
                  ...
                  <div className="bg-white rounded-xl border border-[var(--color-border)] p-5">
                    ...
                    {(() => {
                      ...
                      const labels: Record<string, string> = { mobile: 'Mobil', desktop: 'Desktop' };
                      const colors: Record<string, string> = { mobile: 'bg-purple-500', desktop: 'bg-blue-500' };
                      return Object.entries(analyticsData.deviceBreakdown).map(([device, count]) => (
                        <div key={device} className="mb-3">
                          <div className="flex justify-between text-sm mb-1">
                            <span className="font-medium">{labels[device] || device}</span>
                            <span className="text-[var(--color-text-light)]">{count} ({Math.round(count / total * 100)}%)</span>
                          </div>
                          <div className="w-full bg-gray-100 rounded-full h-2">
                            <div className={`${colors[device] || 'bg-gray-400'} h-2 rounded-full`} style={{ width: `${count / total * 100}%` }} />
                          </div>
                        </div>
                      ));
                    })()}
                    ...
                    {analyticsData.topSearches.length === 0 ? (
                      ...
                      <div key={i} className="flex items-center justify-between py-2 border-b border-[var(--color-border)] last:border-0">
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="text-xs font-bold text-[var(--color-text-light)] w-5">{i + 1}.</span>
                          <span className="text-sm truncate">{s.query}</span>
                        </div>
                        <span className="flex-shrink-0 ml-2 px-2 py-0.5 bg-blue-50 text-[var(--color-primary)] text-xs font-bold rounded-full">{s.count}</span>
                      </div>
                    ))}
                  </div>
                  <div className="bg-white rounded-xl border border-[var(--color-border)] p-5">
                    ...
                    ) : analyticsData.topClicks.map((c, i) => (
                      <div key={i} className="flex items-center justify-between py-2 border-b border-[var(--color-border)] last:border-0">
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="text-xs font-bold text-[var(--color-text-light)] w-5">{i + 1}.</span>
                          <div className="min-w-0">
                            <p className="text-sm truncate">{c.name}</p>
                            <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${c.type === 'afterschool' ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600'}`}>
                              {c.type === 'afterschool' ? 'after school' : 'activitate'}
                            </span>
                          </div>
                        </div>
                        <span className="flex-shrink-0 ml-2 px-2 py-0.5 bg-green-50 text-green-700 text-xs font-bold rounded-full">{c.count}</span>
                      </div>
                    ))}
                  </div>
                </div>
