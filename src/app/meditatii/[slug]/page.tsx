...
import { TUTOR_SUBJECT_LABELS, TUTOR_KIND_LABELS } from '@/lib/tutors';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const db = getDb();
  const p = db.prepare('SELECT * FROM tutors WHERE id = ?').get(id) as Tutor | undefined;
  if (!p) return { title: 'Colaborator negasit' };

  const catLabel = TUTOR_SUBJECT_LABELS[p.subject] || 'Colaborator';
  ...
  const canonical = `https://activkids.ro/meditatii/${toSlug(p.name, p.id)}`;
...
export default async function ProfessionalPage({ params }: Props) {
  ...
  return (
    <>
      <PageviewTracker page={`/meditatii/${slug}`} />
      ...
      <div className="min-h-screen bg-[var(--color-bg)]">
        ...
        <main className="max-w-3xl mx-auto px-4 py-8">
          <div className="bg-[var(--color-card)] rounded-xl shadow-sm border border-[var(--color-border)] overflow-hidden">
            ...
            <div className="p-6">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                ...
                {p.kind && (<span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-semibold border ${p.kind === 'independent' ? 'bg-green-50 text-green-700 border-green-200' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>{TUTOR_KIND_LABELS[p.kind]}</span>)}
          ...
          <div className="mt-6 text-center">
            <a href="/meditatii" className="text-indigo-600 hover:underline text-sm">← Inapoi la meditatii</a>
          </div>
